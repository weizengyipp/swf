...
func TestParsePattern(t *testing.T) {
	...
	ok = ok && reflect.DeepEqual(parsePattern("/p/test/name"), []string{"p", "test", "name"})
...
func TestGetRoute(t *testing.T) {
	r := newTestRouter()
	n, ps := r.getRoute("GET", "/hello/zengyi")
	...
	if ps["name"] != "zengyi" {
		t.Fatal("name should be equal to 'zengyi'")
	}

	fmt.Printf("matched path: %s, params['name']: %s\n", n.pattern, ps["name"])

	n, _ = r.getRoute("GET", "/hell/zengyi")
	if n != nil {
